...
class QEMULauncher:
    ...
    def build_command(self, disk_path, ram_mb=4096, cpu_cores=4, display_type="gtk", 
        ...
        # Physical Disk Passthrough (Native NVMe device for NVMe SSD, IDE/SATA for sdX)
        if "nvme" in disk_path.lower():
            cmd.extend([
                "-drive", f"file={disk_path},format=raw,if=none,id=drive0,cache=writeback",
                "-device", "nvme,drive=drive0,serial=bootbridge_nvme"
            ])
        else:
            cmd.extend([
                "-drive", f"file={disk_path},format=raw,if=ide,index=0,cache=writeback"
            ])
